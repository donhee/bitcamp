// 요청 핸들러의 파라미터 - Cookie 받기(한글일 경우 : URL 인코딩된 쿠키 값을 받을 때는 Cookie 객체로 받아야 문자가 깨지지 않는다.)
...
public class Exam05_8 {
    ...
    public String m2(
            ...
            @CookieValue(value="name", defaultValue="") Cookie name, // 한글 데이터를 쿠키로 받을때는 쿠키로 받아라
            
            // URL 인코딩된 값이 아닌 경우에는 다음과 같이 특정 타입으로(int) 바로 받아도 된다.
            @CookieValue(value="age", defaultValue="0") int age) { // 한글이 아닐 때는 바로 꺼내라 
        try {
