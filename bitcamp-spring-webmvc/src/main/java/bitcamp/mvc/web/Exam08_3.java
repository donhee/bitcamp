...
// Model or Map 객체에 저장하는 데이터 중에서 
// HttpSession 객체에도 저장해야 한다면,
// 다음 애노테이션으로 해당 값의 이름을 지정하라!
@SessionAttributes({"name", "age", "working"})
// 위 애노테이션 의미?
// => name, age 이라는 이름으로 Model or Map 객체에 저장하는 값들을
...
public class Exam08_3 {
    ...
    public String m1(String name, Model model) {
        // 테스트: http://localhost:8888/bitcamp-spring-webmvc/mvc/exam08_3/m1?name=hong
        //       
        // 세션 보관소에 데이터를 직접 저장하는 대신에 Model or Map 객체에 저장한다.
    ...
    public String m2(int age, Model model) {
        // 테스트: http://localhost:8888/bitcamp-spring-webmvc/mvc/exam08_3/m2?age=20
        //       
        // 세션 보관소에 데이터를 직접 저장하는 대신에 Model or Map 객체에 저장한다.
        // => Model이나 Map 객체에 데이터를 저장하면, JSP에서 꺼내 쓸 수 있도록
        //    프론트 컨트롤러가 ServletRequest 보관소로 복사한다.
    ...
    public String m3(boolean working, Model model) {
        // 테스트: http://localhost:8888/bitcamp-spring-webmvc/mvc/exam08_1/m3?working=true
        //       
        // 세션 보관소에 데이터를 직접 저장하는 대신에 Model or Map 객체에 저장한다.
        // => Model이나 Map 객체에 데이터를 저장하면, JSP에서 꺼내 쓸 수 있도록
        //    프론트 컨트롤러가 ServletRequest 보관소로 복사한다.
