...
create table test2(
  ...
  filepath varchar(255) not null, /* 파일시스템에 저장된 첨부파일의 경로 ;파일시스템=폴더 */
...
## FK 제약 조건이 없을 때
- 첨부파일 데이터를 입력할 때 존재하지 않는 게시물 번호가 들어갈수 있다.
- 그러면 첨부파일 데이터를 무효한 데이터 된다.
```
...
- 다른 데이블의 데이터가 참조하는 데이터를 임의로 지우지 못하도록 하는 문법이다.
- 그래서 테이터의 무결성(data integrity; 결함이 없는 상태)을 유지하게 도와주는 문법이다.
