// AOP(Aspect-Oriented Programming) - AOP 필터 적용 후
package bitcamp.java106.step13.ex02;

import org.springframework.context.support.ClassPathXmlApplicationContext;

// AOP?
// => 기존의 코드를 손대지 않고 특정 기능을 삽입하는 기술
// => 메서드 호출 앞뒤로 코드를 삽입할 수 있다.
// => 일종의 필터를 추가하는 기술이다.
//
// AOP 적용 방법
// => AOP 관련 의존 라이브러리 가져온다.
//    - mvnrepository.com에서 "aspectjweaver"로 검색한다.
// => 필터 클래스를 정의한다.
// => XML 또는 자바 코드로 필터를 설정한다.
//
// AOP 용어:
// => Advice        : 메서(Join Point)드 호출 앞뒤에 삽입될 코드이다. 필터 객체이다.
// => Join Point    : Advice가 들어갈 메서드이다.
// => Pointcut      : Advice를 삽입할 위치 정보이다.
// => Target Object : Advice를 삽입할 대상 객체이다.
// => Aspect        : 어느 pointcut에 어떤 advice를 삽입할 것인지 가리키는 정보이다.
//
// 작업 클래스 : ex)
// => Advice        : MyAdvice
// => Join Point    : m() 메서드
// => Pointcut      : execution(* bitcamp.java106.step13.ex02.*.*(..))
// => Target Object : X, Y, Z 클래스
// => Aspect        : 설정 정보

public class Exam01 {
    public static void main(String[] args) {
        ClassPathXmlApplicationContext iocContainer = 
                new ClassPathXmlApplicationContext("bitcamp/java106/step13/ex02/application-context-01.xml");
        
        String[] names = iocContainer.getBeanDefinitionNames();
        for (String name : names) {
            System.out.println(iocContainer.getBean(name).getClass().getName());
        }
        System.out.println("-------------------------------------------");
        
        Caller caller = (Caller) iocContainer.getBean(Caller.class);
        caller.test();
    }
}
