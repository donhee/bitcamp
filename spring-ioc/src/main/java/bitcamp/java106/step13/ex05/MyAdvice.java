...
public class MyAdvice {
    ...
    // <aop:before 
    //      pointcut="execution(* bitcamp.java106.step13.ex05.X.*(..)) and args(a,b)" 
    //      method="doBefore"/>
    //
    @Before("execution(* bitcamp.java106.step13.ex05.X.*(..)) and args(a,b)")
    public void doBefore(int a, int b) {
        System.out.printf("MyAdvice.doBefore(): %d, %d\n", a, b);
    }
    
    // 타겟 객체의 메서드를 호출한 후 그 결과를 받기
    // => 설정 파일에 정의된(32번째라인) 이름을 사용하여 파라미터를 선언해야 한다.
    // <aop:after-returning 
    //      pointcut="execution(* bitcamp.java106.step13.ex05.X.*(..))" 
