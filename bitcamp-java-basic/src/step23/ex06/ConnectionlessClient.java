...
public class ConnectionlessClient {
    public static void main(String[] args) throws Exception {
        // 이 프로그램 실행 방법
        // > java -cp bin step23.ex06.ConnectionlessClient 서버주소 메시지
                                                               
        // connectionless 방식으로 통신을 수행할 소켓 생성
        DatagramSocket socket = new DatagramSocket();
        
        // 데이터를 받을 상대편주소와 포트 번호
        ...
        // 만약 상대편이 연결되어 있지 않다면, 보낸 데이터는 버려진다. (그쪽 네트웍에서 버려진다)
        // => 따라서 데이터 송수신을 보장하지 않는다. (connectionless 방식)
    }
}
