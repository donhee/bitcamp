...
- JSP를 만들 당시에는 JSP 페이지를 작성할 때 Java 언어 외에 다른 언어를 사용할 것을 대비해서
  language라는 속성을 만들었다.
   ...
   지금은 Java만 넣는다!!!!!!
   그래서 language 속성에 값으로는  page language="java"만 가능하다!!!!!
   language 속성은 생략해도 된다.

[contentType 속성]
- response.setContextType() 코드를 만든다.
...
- 지시문 사이의 있는 공백(스페이스, 탭, 줄바꿈)을 제거한다. => 값을 true로 설정하면
</pre>
...
[autoFlush 속성]
- 기본값은 true 이다. 즉 버퍼가 꽉 차면 바로 클라이언트로 출력한다.
- false로 설정되어 있다면, 버퍼가 꽉 찼을 때 IOException 오류가 발생한다.
