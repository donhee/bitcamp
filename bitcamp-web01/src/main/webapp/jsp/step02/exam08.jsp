...
별명
  ...
  - JSP 기본 라이브러리나 유명 라이브러리를 사용할 때는 가능한 다른 개발자들이 
     관습적으로 사용하는 별명을 지정하라! (개성을 살리지 말 것!)
[동작]
- JSP에서 기본으로 제공하는 태그 외에 특별한 작업을 수행하는 태그를 사용할 수 있다.
- 즉 해당 태그를 사용하면 특별한 작업을 수행하는 자바 코드를 생성한다.

[JSP 확장 태그를 사용하기 위해 준비해야 할 일은?]
- 확장 태그가 정의된 파일과 그 태그를 처리하는 자바 클래스가 들어 있는 라이브러리를 가져와야 한다.
- 그 라이브러리를 WEB-INF/lib 폴더에 배치시켜라!
1) mvnrepository.com 에서 "jstl" 라이브러리 검색한다.
2) build.gradle에 의존 라이브러리 추가한다.
3) "gradle eclipse" 실행한다.
4) 이클립스 프로젝트 Refresh 한다.
4) tomcat 서버 다시 Restart 한다.
5) 테스트 해본다.
</pre>
