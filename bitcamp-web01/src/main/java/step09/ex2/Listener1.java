...
// => 예) 애플리케이션 성능 관리(=모니터링) 시스템(Application Performance Monitoring System; APM)
//
...
//
// 리스너 만들기
// => 이벤트가 발생했을 때 어떤 메서드를 호출할지 서블릿 기술에 명세되어 있다.
...
// 배치하는 방법
// 1) 애노테이션을 사용하여 배치하는 방법
// => @WebListener
// 2) web.xml 파일에 배치 정보 설정하는 방법
//   <listener>
...
public class Listener1 implements ServletContextListener{
    ...
    public void contextInitialized(ServletContextEvent sce) {
        // 이 메서드는 서블릿 컨테이너가 시작될 때 호출된다.
        // => 여기에는 모든 서블릿들이 사용할 도구를 준비시키는 코드를 두면 된다.
        System.out.printf("Listener1.contextInitializaed() - %s\n", new Date());
    }
    
    @Override
    public void contextDestroyed(ServletContextEvent sce) {
        // 이 메서드는 서블릿 컨테이너가 종료될 때 호출된다.
