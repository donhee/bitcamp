...
// 하는 일:
// => 클라이언트가 요청이 들어오면 그 요청을 Controller에게 위임하는 역할
// => 스프링 IoC 컨테이너를 사용하여 Controller 및 DAO 등을 관리한다.
//
...
public class DispatcherServlet implements Servlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        // 이 메서드는 클라이언트가 최초의 요청이 들어오면 딱 한 번 실행한다.
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        // 클라이언트 요청이 들어올 때마다 서블릿 컨테이너가 호출하는 메서드
        ...
        Controller controller = (Controller) iocContainer.getBean(path); //  /board/list 
    ...
    public void destroy() {
        ...
        //    예를 들면 파일이나 DB 커넥션, 소켓 등의 자원을 해제한다.
    }
...
//    => 프로젝트/bin/main/모든것을 --> WEB-INF/classes/* 복사한다.
// 4) 외부 라이브러리를 배치한다.
...
// => 이 라이브러리를 배포하려면 gradle을 통해 라이브러리를 복사해야 한다.
//    어떻게?
//    > gradle build 실행하라!!!
//    그러면 build/
//
