...
학습목표
    ...
    - Path Variable을 사용할 수 있다.(list나 view)
작업내용
...
학습목표
    ...
    - 스프링 WebMVC 프론트 컨트롤러를 프로젝트에 적용할 수 있다.
    - 스프링 WebMVC 규칙에 따라 페이지 컨트롤러를 만들 수 있다.
...
작업내용
    ...
    - 문자열 파라미터 값을 java.sql.Date 객체로 변환시키는 커스텀 프로퍼티 에디터(GlobalBindingInitializer)를 등록한다.
...
## src46 - page Controller를 POJO로 변경한다.
학습목표
    - POJO의 의미를 이해한다.
...
학습목표
    ...
    - 서블릿 오류에 대한 전용 페이지 설정할 수 있다. (web.xml)
작업내용
    - 컨트롤러들의 공통 기능을 모아 Front Controller에게 위임한다.
...
학습목표
    ...
    - 컨텍스트 파라미터<context-param>를 설정하고 사용할 수 있다.
작업내용
    - Filter에서 클라이언트 데이터 인코딩 지정을 수행한다.(CharacterEncodingFilter)
    - ServletContextListener 구현체에서 Spring IoC 컨테이너 준비(ContextLoaderListener, WebApplicationContextUtils)
...
## src35 - Spring 프레임워크 IoC 컨테이너 적용
학습목표
...
작업내용
    - 기존의 테이블에서 FK를 적용한 새로운 테이블을 만든다.
    - SQL 맵퍼 파일을 변경한다.
...
## src32 - DB Connection Pool을 적용하여 DB 연결 객체를 효율적으로 관리하라.
학습목표
    - "pooling 기법(일정 수량의 객체를 만들어 놓고 재사용하는 방법)"을 이해한다.
    - "flyweight 디자인 패턴"을 이해하고 구현할 수 있다.
    - "proxy 디자인 패턴"을 이해하고 구현할 수 있다.
    - DB Connection Pool 개념을 이해하고 구현할 수 있다.
작업내용
    - DB 커넥션에 대해 "pooling 기법"을 적용하여 DB Connection Pool을 만든다.
    - Connection 객체에 close() 메서드를 재정의하기 위해 "proxy 패턴"을 적용한다. 
...
학습목표 
    ...
    - 파일 입출력에 대한 예외 처리 방법을 익힌다.
작업내용
    - 도메인 클래스에 serialize 기능을 활성화 시킨다.
...
작업내용
    - 기존의 컨트롤러 클래스를 메서드 단위로 쪼개서 클래스로 분리한다.
